...
tar -cf - installer firmware.tar.gz | ssh -o ConnectTimeout=10 \
    ...
    -o StrictHostKeyChecking=accept-new "root@$router" \
    "set -e; mkdir /tmp/be7000-install.lock; trap 'rmdir /tmp/be7000-install.lock' EXIT; touch /tmp/be7000-installing; mkdir -p /tmp/be7000-autostart.lock /tmp/be7000-snapshot-install; tar -xf - -C /tmp/be7000-snapshot-install; sh /tmp/be7000-snapshot-install/installer/router-install.sh $auto"
