...
EXPECTED_KERNEL_ENTRY="0x42000000"
...
[ "${1:-}" = "LOAD-LINUX612-V@RUN@" ] || {
	...
	echo "Explicit syntax: $0 LOAD-LINUX612-V@RUN@"
	exit 2
}
...
insmod "$ARCH_MODULE" detect_el2=1 shim_hyp=0 probe_idmap=1 holding_pen=@HOLDING_PEN@
LOADED_ARCH=1
insmod "$CORE_MODULE"
LOADED_CORE=1
...
[ -r "$BASE_DIR/launch.conf" ] || die "missing launch.conf"
...
FINAL_CMDLINE="console=ttyMSM0,115200n8 uart_en=1 root=/dev/ram0 rdinit=/usr/libexec/be7000-usb-init boot_source=kexec kexec_quiesce=1 maxcpus=4 be7000_printk=1 be7000_handoff=1 be7000_source=linux612-v@RUN@ be7000_usb_uuid=$USB_UUID be7000_usb_dir=@USB_DIR@ earlycon loglevel=7 pcie_pme=nomsi be7000_diagnostic=$DIAGNOSTIC"
...
grep -q '^segment\[0\]\.mem   = 0x42000000$' "$LOAD_LOG" ||
...

{
	...
	echo "build=$BUILD_TAG"
	echo "source_kernel_profile=$KERNEL_PROFILE"
	echo "target=openwrt-usb-linux612-v@RUN@"
	...
	echo "image_probe=linux612-v@RUN@:native-openwrt:printk-ring-rsvd1:serialized-rpm-glink-handoff"
	...
	echo "second_kernel_watchdog=openwrt-procd"
	echo "smp_diagnostic=maxcpus-4:physical-spin-table-handoff"
	...
	echo "physical_layout=arm64-image-0x42000000"
...
SUCCESS=1
echo "OpenWrt Linux 6.12 with embedded initramfs is loaded into RAM."
echo "Verified placement: kernel=$EXPECTED_KERNEL_ENTRY dtb=$EXPECTED_DTB purgatory=$EXPECTED_PURGATORY"
echo "The physical spin-table handoff with maxcpus=4 is armed; no transition was performed."
