...
BUILD_TAG="be7000-kexec-v63-linux612-v@RUN@-ram"
SELF_DIR=${0%/*}
[ "$SELF_DIR" = "$0" ] && SELF_DIR=.
...
be7000_kernel_profile() {
    ...
    case "$(uname -v)" in
        '#0 SMP PREEMPT Mon Jan 22 02:33:42 2024') KERNEL_PROFILE=20240122; KERNEL_FIRMWARE=1.1.16; KERNEL_PEN=ffffffc0107f51a4;;
        '#0 SMP PREEMPT Tue Jan 27 03:33:27 2026') KERNEL_PROFILE=20260127; KERNEL_FIRMWARE=1.1.38; KERNEL_PEN=ffffffc0107f61a4;;
...
[ "${1:-}" = "EXECUTE-KEXEC-QUIESCED" ] || {
	...
	echo "then attempts to start the prepared native OpenWrt USB kernel."
...
grep -q '^target=openwrt-usb-linux612-v@RUN@$' "$BASE_DIR/loaded-state.txt" ||
	die "loaded target is not the linux612-v@RUN@ RAM system"
...
grep -q '^kernel_entry=0x42000000$' "$BASE_DIR/loaded-state.txt" ||
...
LAYOUT=$("$BASE_DIR/bash-runtime/lib/ld-musl-aarch64.so.1" --library-path "$BASE_DIR/bash-runtime/lib" "$BASE_DIR/bash-runtime/bin/bash" "$BASE_DIR/06-check-layout.sh" "$LOAD_LOG") || die "invalid loaded segment layout"
set -- $LAYOUT
...
grep -qx 'image_bytes=@IMAGE_BYTES@' "$BASE_DIR/loaded-state.txt" || die "wrong Image size record"
RECORDED_CMDLINE=$(sed -n 's/^cmdline=//p' "$BASE_DIR/loaded-state.txt")
for arg in root=/dev/ram0 rdinit=/usr/libexec/be7000-usb-init be7000_printk=1 be7000_handoff=1 be7000_source=linux612-v@RUN@ maxcpus=4; do
	case " $RECORDED_CMDLINE " in
		*" $arg "*) ;;
		*) die "missing target boot argument: $arg" ;;
	esac
done
grep -q '^image_probe=linux612-v@RUN@:native-openwrt:printk-ring-rsvd1:serialized-rpm-glink-handoff$' "$BASE_DIR/loaded-state.txt" ||
...
grep -q '^second_kernel_watchdog=openwrt-procd$' "$BASE_DIR/loaded-state.txt" ||
	die "the clean-control watchdog policy was not selected"
grep -q '^smp_diagnostic=maxcpus-4:physical-spin-table-handoff$' "$BASE_DIR/loaded-state.txt" ||
...
grep -q '^physical_layout=arm64-image-0x42000000$' "$BASE_DIR/loaded-state.txt" ||
...
strings "$BASE_DIR/kexec_mod_arm64.ko" |
	grep -q 'be7000_spin_table=code-4fb3e000-release-4fb3eff8-entry-module-parameter' ||
...
BASH_RUNTIME=/tmp/be7000-linux612-runtime
[ ! -e "$BASH_RUNTIME" ] || die "temporary Bash runtime already exists"
cp -a "$BASE_DIR/bash-runtime" "$BASH_RUNTIME"
...
# Local only: keep Xiaomi's saved panic on this router's USB drive.
if [ "$USB_MOUNT" != none ]; then
(
	trap - EXIT HUP INT TERM
	PANIC_SOURCE=/data/usr/log/panic.tar.gz
	PANIC_BOOT_DONE=/tmp/boot_check_done
	PANIC_LOCK=/var/run/upcrash.lock
	command -v lock >/dev/null && command -v cmp >/dev/null || exit 1
	waited=0
	while [ ! -f "$PANIC_BOOT_DONE" ]; do
		[ "$waited" -lt 30 ] || exit 1
		sleep 1
		waited=$((waited + 1))
	done
	# boot_check starts upcrash in the background after a five-second sleep.
	sleep 6
	waited=0
	while ! lock -n "$PANIC_LOCK"; do
		[ "$waited" -lt 30 ] || exit 1
		sleep 1
		waited=$((waited + 1))
	done
	trap 'lock -u "$PANIC_LOCK"' EXIT
	trap 'exit 1' HUP INT TERM
	[ -s "$PANIC_SOURCE" ] || { echo "No saved Xiaomi panic archive"; exit 0; }
	PANIC_USB_DIR="$BASE_DIR/logs/panic"
	prefix=previous
	[ "$PANIC_SOURCE" -nt "$PANIC_BOOT_DONE" ] && prefix=panic
	PANIC_USB_FILE="$PANIC_USB_DIR/$prefix-$STAMP.tar.gz"
	if mkdir -p "$PANIC_USB_DIR" &&
		cp "$PANIC_SOURCE" "$PANIC_USB_FILE.new" &&
		mv -f "$PANIC_USB_FILE.new" "$PANIC_USB_FILE" &&
		sync && cmp -s "$PANIC_SOURCE" "$PANIC_USB_FILE"; then
		rm -f "$PANIC_SOURCE" || exit 1
		sync || exit 1
		echo "Xiaomi panic saved to USB and removed from /data: $PANIC_USB_FILE"
	else
		exit 1
	fi
) || echo "WARNING: panic archive transfer incomplete; continuing boot" >&2
fi
