...
static int probe_idmap = 0;
module_param(probe_idmap, int, 0);
MODULE_PARM_DESC(probe_idmap,
		 "Run a reversible physical idmap call during module load");
...
kexecmod_arm64_init(void)
{
	...
	err = kexec_idmap_setup();
	if (err) {
		pr_err("Failed to build identity map: %d\n", err);
		machine_kexec_compat_unload();
		return err;
	}

	err = kexec_secondary_park_init();
	if (err) {
		pr_err("Failed to prepare secondary CPU handoff: %d\n", err);
		machine_kexec_compat_unload();
		return err;
	}

	if (probe_idmap) {
		err = kexec_idmap_selftest();
		if (err) {
			pr_err("Identity-map self-test failed: %d\n", err);
			kexec_secondary_park_exit();
			machine_kexec_compat_unload();
			return err;
		}
	}
