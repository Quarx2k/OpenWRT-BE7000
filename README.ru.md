...
Нужны root-доступ по SSH и **флешка ext4 с 3 ГиБ свободного места**, подключённая к роутеру и смонтированная в Xiaomi. Для swap потребуется дополнительное место. Поддерживаемое ядро Xiaomi:
...
В **LuCI → Система → Резервное копирование / Прошивка** выберите файл `BE7000-OpenWrt-1.0.0-sysupgrade.bin`.
...
| Файл | Назначение |
| --- | --- |
| `BE7000-OpenWrt-1.0.0.tar.gz` | Образ для установщика |
| `BE7000-OpenWrt-1.0.0-sysupgrade.bin` | Обновление через LuCI |
| `BE7000-runtime-1.0.0.tar.gz` | Компоненты для повторной сборки с `--runtime-kit` |
