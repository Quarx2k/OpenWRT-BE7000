...
Нужны root-доступ по SSH и **флешка ext4 с 3 ГиБ свободного места**, подключённая к роутеру и смонтированная в Xiaomi. Для swap потребуется дополнительное место. Установщик автоматически распознаёт эти ядра Xiaomi:
...
В **LuCI → Система → Резервное копирование / Прошивка** выберите файл `BE7000-OpenWrt-1.0.1-sysupgrade.bin`.

Настройки можно сохранить или сбросить. Пакеты, которые вы добавили сами, нужно будет установить заново. Калибровки, файлы Wi-Fi и swap сохранятся в любом случае.

Для первого обновления нужны ещё 3 ГиБ на флешке: новая версия записывается рядом с предыдущей. Старые установки без каталога `slots` нужно переустановить.
...
| Файл | Назначение |
| --- | --- |
| `BE7000-OpenWrt-1.0.1.tar.gz` | Образ для установщика |
| `BE7000-OpenWrt-1.0.1-sysupgrade.bin` | Обновление через LuCI |
| `BE7000-runtime-1.0.1.tar.gz` | Компоненты для повторной сборки с `--runtime-kit` |
